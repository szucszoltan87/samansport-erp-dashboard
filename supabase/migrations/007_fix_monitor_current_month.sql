-- ============================================================
-- FIX: Készlet Monitor — use latest data month instead of
-- CURRENT_DATE for "Havi el." and "Havi hátra" columns.
--
-- Problem: when data sync lags behind CURRENT_DATE, the
-- cur_month CTE finds no rows → all "Havi el." values are 0.
--
-- Solution: detect the latest year/month with actual sales
-- data for the tenant and use that as the reference month.
-- Forecast months (H+1/H+2/H+3) shift accordingly.
-- ============================================================
...
BEGIN
    ...
    SELECT TO_DATE(MAX(s.telj_d), 'YYYY.MM.DD')
     ...
     WHERE s.tenant_id = p_tenant_id
       AND s.telj_d IS NOT NULL AND s.telj_d <> ''
       AND s.cikktipus = 'T';

    -- Use latest data month, falling back to CURRENT_DATE if no data at all
    IF v_latest_date IS NOT NULL THEN
        v_cur_year  := EXTRACT(YEAR  FROM v_latest_date)::int;
        v_cur_month := EXTRACT(MONTH FROM v_latest_date)::int;
    ELSE
        v_cur_year  := EXTRACT(YEAR  FROM CURRENT_DATE)::int;
        v_cur_month := EXTRACT(MONTH FROM CURRENT_DATE)::int;
    END IF;
